...
`BtechApp.open()` registers a targeted wait for the initial regions request before navigation, requires its `200` response, and then waits for the rendered `Delivering to:` header state, its expected location, and an editable search input. The expected location can be passed to `BtechApp` and defaults to `Cairo`. This is a narrow application-readiness barrier, not a replacement for UI assertions and not a claim that the regions request alone enables search.
